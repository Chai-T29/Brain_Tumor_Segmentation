...
- **Polygonal Environment with Dynamic Centre:** The environment models a configurable `N`-sided polygon (default 32) whose alternating supporting lines are actively controlled. Each line receives two continuous commands—radial motion and angular offset—while two additional actions shift the polygon centre directly along the `x`/`y` axes (all commands lie in `[-1, 1]`) and a shared stop signal terminates the episode. IoU with the tumour mask is computed by rasterising the polygon, and rewards remain tied to IoU deltas with the original terminal bonuses. Ground-truth targets now anchor the polygon at the tumour centroid so guided rollouts track the lesion more faithfully.
- **TD3 with n-step Targets:** Actor and the twin critics now maintain their own lightweight embedding encoders on top of the cached EfficientNet feature maps. Targets incorporate configurable n-step returns, Polyak averaging, target policy smoothing, and delayed policy updates.
- **Replay-Friendly Embeddings:** The replay buffer can store compact pointers back to memmapped embedding volumes, dramatically reducing RAM footprint while still serving tensors on demand during optimisation.
...
- **Polygon geometry:** `environment.num_sides` (default 32) determines control dimensionality (`num_sides*2 + 2 + 1` = per-line distance/angle pairs, two centre-shift axes, and a stop gate). Radial and angular step sizes are governed by `line_distance_step_scale` and `line_angle_step_scale_deg`, while `center_step_scale` tunes how quickly the polygon’s origin can move per step.
